...
int last_win(std::vector<Board> &boards, const std::vector<int> &numbers, int *index) {
	int numbers_added = 0, boards_left = (int)boards.size();
	for (int num : numbers) {
		for (*index = 0; *index < (int)boards.size(); ++*index) {
			boards[*index].check_value(num);
			if (++numbers_added >= 5 && boards[*index].new_bingo()) {
				//std::cout << "Board " << *index << " wins on " << num << '\n';
				if (boards_left == 1)
...
int main() {
	...
	int index, score = last_win(boards, numbers, &index);
