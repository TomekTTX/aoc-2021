...
public:
    ...
    inline bool was_visited() const { return visited; }
    inline void visit() { visited = !large; /*std::cout << name_ << " visited\n";*/ }
    inline void unvisit() { visited = false; /*std::cout << name_ << " unvisited\n";*/ }
...
private:
    void find_paths_rec(GraphNode &node, std::vector<GraphNode *> &path, std::vector<std::vector<GraphNode *>> &paths) const {
